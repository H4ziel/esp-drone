...
void pwm_task(void* pvParameters){
    ...
    while(true){
        ...
        duty_att(dc1, PWM1_TIMER, PWM1_CHANNEL);
        duty_att(dc2, PWM2_TIMER, PWM2_CHANNEL);

        ESP_LOGI(TAG_PWM,"dc1: %ld", ledc_get_duty(LEDC_HIGH_SPEED_MODE,
                                                                PWM1_CHANNEL));
...
void mpu_task(void* pvParameters){
    ...
    while(1){
        ...
        ESP_LOGI(TAG_MPU, "pitch=%.1f roll=%.1f yaw=%.1f", Kfilter_pitch->x[0], Kfilter_roll->x[0], mpu->yaw);
